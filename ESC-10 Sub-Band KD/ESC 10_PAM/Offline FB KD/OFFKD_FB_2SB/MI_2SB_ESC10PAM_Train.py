...
import warnings
from Pathname import FB_PAM_Path,SB21_PAM_Path,SB22_PAM_Path,S1_PAM_MI_Path
...
def MINEN_Loss(Predicted_FB_Label,Predicted_SB1_Label,Predicted_SB2_Label,Predicted_Student_Label,T,N_models,y,device,criterion):
...
def train(FB_Model, SB1_Model, SB2_Model,Student_Model,device,iterator, optimizer, criterion,alpha,Temp): # Define Training Function 
    ...
    for (a,b,c,y) in iterator:
        ...
        EN_AVG_Loss=MINEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_Train_Label,Temp,N_models,y,device,criterion)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S1_PAM_MI_Path
...
np.save('S1_PAM_MI_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
