...
alpha=0.3
N_models=5
from Pathname import FB_PAM_Path, SB41_PAM_Path,SB42_PAM_Path,SB43_PAM_Path,SB44_PAM_Path,S3_PAM_WE_Path
...
def WEIEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_SB4_Label,Predicted_Student_Label,T,N_models,y,device):
        ...
        SB4_Prob=F.softmax(Predicted_SB4_Label/T,dim=1)
        ...
        SB4_Loss=criterion(Predicted_SB4_Label,y)
        W1=torch.exp(FB_Loss)
        W2=torch.exp(SB1_Loss)
        W3=torch.exp(SB2_Loss)
        W4=torch.exp(SB3_Loss)
        W5=torch.exp(SB4_Loss)
        FW1=1-((W1)/(W1+W2+W3+W4+W5))
        FW2=1-((W2)/(W1+W2+W3+W4+W5))
        FW3=1-((W3)/(W1+W2+W3+W4+W5))
        FW4=1-((W4)/(W1+W2+W3+W4+W5))
        FW5=1-((W5)/(W1+W2+W3+W4+W5))
        AVG_Prob=(FW1*FB_Prob+FW2*SB1_Prob+FW3*SB2_Prob+FW4*SB3_Prob+FW5*SB4_Prob)
...
def train(FB_Model, SB1_Model, SB2_Model,SB3_Model,SB4_Model,Student_Model,device,iterator, optimizer, criterion,alpha,Temp): # Define Training Function 
    ...
    for (a,b,c,d,e,y) in iterator:
        ...
        EN_AVG_Loss=WEIEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_SB4_Label,Predicted_Train_Label,Temp,N_models,y,device)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S3_PAM_WE_Path
...
np.save('S3_PAM_WE_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
