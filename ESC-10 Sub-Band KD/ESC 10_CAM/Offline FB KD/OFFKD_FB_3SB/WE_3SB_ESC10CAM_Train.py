...
from ESC10Customdataset_Train_3SB1 import LAEData_Train # Call Customdataloader to read Training Data
from ESC10Customdataset_Test_3SB1 import LAEData_Test # Call Customdataloader to read Test Data
...
from Pathname import FB_CAM_Path, SB31_CAM_Path,SB32_CAM_Path,SB33_CAM_Path,S2_CAM_WE_Path
import warnings
...
def WEIEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_Student_Label,T,N_models,y,device):
        ...
        SB3_Loss=criterion(Predicted_SB3_Label,y)
        W1=torch.exp(FB_Loss)
        W2=torch.exp(SB1_Loss)
        W3=torch.exp(SB2_Loss)
        W4=torch.exp(SB3_Loss)
        FW1=1-((W1)/(W1+W2+W3+W4))
        FW2=1-((W2)/(W1+W2+W3+W4))
        FW3=1-((W3)/(W1+W2+W3+W4))
        FW4=1-((W4)/(W1+W2+W3+W4))
        AVG_Prob=(FW1*FB_Prob+FW2*SB1_Prob+FW3*SB2_Prob+FW4*SB3_Prob)
...
def calculate_accuracy(fx, y): # caluate accuracy 
    ...
    return acc
def train(FB_Model, SB1_Model, SB2_Model,SB3_Model,Student_Model,device,iterator, optimizer, criterion,alpha,Temp): # Define Training Function 
    ...
    for (a,b,c,d,y) in iterator:
        ...
        EN_AVG_Loss=WEIEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_Train_Label,Temp,N_models,y,device)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S2_CAM_WE_Path
...
for epoch in range(EPOCHS):
    ...
    train_loss, train_acc = train(FB_Model, SB1_Model, SB2_Model,SB3_Model,Student_Model,device,train_loader,Student_optimizer,criterion,alpha,Temprature) # Call Training Process 
    ...
    early_stopping(valid_loss,Student_Model,S2_CAM_WE_Path) # call Early Stopping to Prevent Overfitting 
    if early_stopping.early_stop:
        print("Early stopping")
        break
    Student_Model.load_state_dict(torch.load(MODEL_SAVE_PATH))
np.save('S2_CAM_WE_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
