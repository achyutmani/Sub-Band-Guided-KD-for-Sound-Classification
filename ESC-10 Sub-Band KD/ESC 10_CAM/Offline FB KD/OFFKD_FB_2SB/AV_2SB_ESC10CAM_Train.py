...
import warnings
from Pathname import FB_CAM_Path,SB21_CAM_Path,SB22_CAM_Path,S1_CAM_AV_Path
...
def AVGEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_Student_Label,T,N_models,y,device):
        ...
        Student_Prob=F.log_softmax(Predicted_Student_Label/10,dim=1)
...
def calculate_accuracy(fx, y): # caluate accuracy 
    ...
    return acc
def train(FB_Model, SB1_Model, SB2_Model,Student_Model,device,iterator, optimizer, criterion,alpha,Temp): # Define Training Function 
    ...
    for (a,b,c,y) in iterator:
        ...
        EN_AVG_Loss=AVGEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_Train_Label,Temp,N_models,y,device)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S1_CAM_AV_Path
...
for epoch in range(EPOCHS):
    ...
    train_loss, train_acc = train(FB_Model, SB1_Model, SB2_Model,Student_Model,device,train_loader,Student_optimizer,criterion,alpha,Temprature) # Call Training Process 
...
np.save('S1_CAM_AV_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
