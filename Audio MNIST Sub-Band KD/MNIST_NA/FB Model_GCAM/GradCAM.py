...
from Pathname import FB_SA_Path
...
key='0_george_20.wav'
with h5py.File('MNIST.hdf5', 'r') as f: # Database for Left Channel Test Spectrogram 
            SG_Data = f[key][()]
...
plt.yticks(size=25)
plt.subplot(1,2,2)
librosa.display.specshow(class_activation[:,0:431],sr=44100,y_axis='linear',cmap='plasma');
plt.ylabel("Frequency",size=25)
plt.xlabel("Time",size=25)
...
plt.savefig('/mnt/4882F3AC82F39D1E/Journal WOrk Codes/KD/ESC-10 SB Work/ESC 10_NA/FB Model_GCAM/GCAM MNIST/0_Attention_Map_New.png', bbox_inches='tight') # bbox_inches removes extra white spaces
plt.show()
