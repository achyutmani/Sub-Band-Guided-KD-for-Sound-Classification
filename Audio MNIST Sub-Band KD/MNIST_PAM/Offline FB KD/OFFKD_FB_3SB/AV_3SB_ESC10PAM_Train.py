...
from Pathname import FB_PAM_Path, SB31_PAM_Path,SB32_PAM_Path,SB33_PAM_Path,S2_PAM_AV_Path
import warnings
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S2_PAM_AV_Path
...
for epoch in range(EPOCHS):
    ...
    early_stopping(valid_loss,Student_Model,S2_PAM_AV_Path) # call Early Stopping to Prevent Overfitting 
    if early_stopping.early_stop:
        print("Early stopping")
        break
    Student_Model.load_state_dict(torch.load(MODEL_SAVE_PATH))
np.save('S2_PAM_AV_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
