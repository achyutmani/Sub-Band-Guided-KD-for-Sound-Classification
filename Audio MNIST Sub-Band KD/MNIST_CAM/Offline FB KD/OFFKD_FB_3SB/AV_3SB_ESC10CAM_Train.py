...
from Pathname import FB_CAM_Path, SB31_CAM_Path,SB32_CAM_Path,SB33_CAM_Path,S2_CAM_AV_Path
import warnings
...
def AVGEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_Student_Label,T,N_models,y,device):
        ...
        AVG_Prob=(FB_Prob+SB1_Prob+SB2_Prob+SB3_Prob)/N_models
...
def train(FB_Model, SB1_Model, SB2_Model,SB3_Model,Student_Model,device,iterator, optimizer, criterion,alpha,Temp): # Define Training Function 
    ...
    for (a,b,c,d,y) in iterator:
        ...
        EN_AVG_Loss=AVGEN_Loss(Predicted_FB_Label,Predicted_SB1_Label, Predicted_SB2_Label,Predicted_SB3_Label,Predicted_Train_Label,Temp,N_models,y,device)
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = S2_CAM_AV_Path
...
for epoch in range(EPOCHS):
    ...
    early_stopping(valid_loss,Student_Model,S2_CAM_AV_Path) # call Early Stopping to Prevent Overfitting 
    if early_stopping.early_stop:
        print("Early stopping")
        break
    Student_Model.load_state_dict(torch.load(MODEL_SAVE_PATH))
np.save('S2_CAM_AV_ESC10_Model_Parameters',Temp) # Save Temp Array as numpy array 
